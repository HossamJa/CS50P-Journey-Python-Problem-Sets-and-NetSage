...
import requests, subprocess, socket, ssl, types
...
def test_check_speed(monkeypatch):
    ...
    monkeypatch.setattr('project.speedtest.Speedtest', MockSpeedtest)
    result = project.check_speed()
...
def test_get_ISPndLoc_info():
    ...
    def mock_get(*args, **kwargs):
        return MockResponse()
    import requests
    from project import get_ISPndLoc_info
    # Patch requests.get
    orig_get = requests.get
    requests.get = mock_get
    result = get_ISPndLoc_info()
...
def test_get_compareson():
    ...
    project.user_country = "US"
    project.user_download_speed = 100.0
    project.user_upload_speed = 50.0
    project.user_ping = 10.0
    ...
    def mock_bs(*args, **kwargs):
        return MockSoup()
    project.BeautifulSoup = mock_bs

    result = project.get_compareson()
...
def test_check_Wifi_quality():
    ...
    result = project.check_Wifi_quality()
...
def test_check_website_stat():
    ...
    orig_check_internet = project.check_internet
    project.check_internet = lambda: {"net_status": "Online", "error": "", "suggests": ""}
    ...
    import builtins
    orig_whois = getattr(project, "whois", None)
    project.whois = types.SimpleNamespace(whois=lambda domain: MockWhois())
    ...
    result = project.check_website_stat("example.com")
    ...
    subprocess.Popen = orig_popen
    if orig_whois: project.whois = orig_whois
    ...
    project.check_internet = orig_check_internet
